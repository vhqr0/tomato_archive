...
void ClientSession::bind(uint16_t lport, uint16_t rport) {
  lport_ = lport;
  rport_ = rport;
  LOG_MSG("bind " + std::to_string(lport) + " to " + std::to_string(rport));
  std::memcpy(&out_buf_[0], config.password, 16);
  out_buf_[16] = 5;
  out_buf_[17] = 2;
  out_buf_[18] = 0;
  if (config.client_remote.address().is_v4()) {
    length_ = 26;
    out_buf_[19] = 1;
    auto addr = config.client_remote.address().to_v4().to_bytes();
    std::memcpy(&out_buf_[20], &addr[0], 4);
    out_buf_[24] = rport >> 8;
    out_buf_[25] = rport & 0xff;
  } else if (config.client_remote.address().is_v6()) {
    length_ = 38;
    out_buf_[19] = 4;
    auto addr = config.client_remote.address().to_v6().to_bytes();
    std::memcpy(&out_buf_[20], &addr[0], 16);
    out_buf_[36] = rport >> 8;
    out_buf_[37] = rport & 0xff;
  } else {
                          ...
                          asio::async_read( // receive socks response 2
                            ...
                            [this, self](asio::error_code ec, std::size_t length) {
                              ...
                              socket_.async_connect( // connect
                                asio::ip::tcp::endpoint(config.client_local.address(), lport_),
